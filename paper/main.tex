...
Terminal identity also has a user-visible cost: $c_{i,3}^{\pi}=s_{i,3}^{\pi}$ can replace an already correct commitment with an incorrect complete-modality prediction. O1 makes this boundary explicit. Because O1 commits before the terminal stage only when the provisional state equals the label, its 14.33\% revision-per-path rate is induced by forced terminal replacement; whenever the terminal TAV state is wrong, that replacement degrades a previously correct visible state. \method{} therefore guarantees comparability with the frozen terminal predictor, not monotonic correctness of the visible trajectory. A deployment that retains a correct earlier state would require a different terminal contract and would no longer preserve terminal identity.
