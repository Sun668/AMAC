...
def main():
    ...
    root = Path(__file__).resolve().parents[3]; output = Path(args.output).resolve()
    if output.exists(): raise RuntimeError("冻结快照已存在，禁止覆盖")
    value = json.loads(Path(args.template).read_text(encoding="utf-8"))
    ...
    reference = root / value["models"]["control"]["artifact"]
    value["models"]["control"]["artifact_sha256"] = sha256(reference)
    value["environment"].update({"python": platform.python_version(), "numpy": numpy.__version__, "sklearn": sklearn.__version__, "torch": torch.__version__})
    value["status"] = "frozen"; output.parent.mkdir(parents=True, exist_ok=False)
