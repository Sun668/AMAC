...
def main():
            ...
            for risk_seed in sorted({row[0] for row in rows}):
                ...
                if scope == "source_disjoint_test":
                    ...
                    for index, source in enumerate(ordered):
                        ...
                        clip_count = len({row[1] for row in disjoint_rows
                                          if row[2] == source and row[3] == "B4"})
                        if int(recorded["clips"]) != clip_count:
    ...
    if (decision.get("valid_result") != all(validity.values()) or
        ...
        errors.append("决策错误")
    report = {
        "schema": "amac-cluster-statistics-validator-v3", "status": "failed" if errors else "passed",
